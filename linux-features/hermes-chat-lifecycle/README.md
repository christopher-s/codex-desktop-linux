...
The renderer does not perform a standalone lifecycle `probe` on the foreground Chat submission path. Eligibility is resolved by `begin_turn` in the trusted main process. Project-less ordinary Chat is eligible when the feature is enabled. Non-empty Custom GPT/Gizmo IDs still fail closed unless registered, and unrelated webviews keep their normal behavior. User-turn identity and visible user text are captured synchronously at the original turn boundary before upstream request preparation can reuse minified locals. The late `begin_turn`, pre-API, and terminal lifecycle phases consume those stable captures, including on reopened/resumed request shapes. The `probe` phase remains available for diagnostics and QA controls.
...
Successful completion ordering is deliberate: the native ChatGPT success comma-chain keeps its original final expression and return value. The renderer structurally locates that `completed && (...)` chain and inserts a versioned `queueMicrotask` terminal hook immediately before its final top-level expression. The native finalizer therefore still runs synchronously and remains the comma-chain result, while Hermes `complete_turn` begins on the following microtask after synchronous native completion. Already-patched candidates with notify-before-cleanup, mid-chain terminal hooks, or the V2 after-tail hook are migrated structurally and idempotently; malformed or ambiguous V3 placement fails closed. Live E1 also proved that ordering alone does not populate assistant history. `assistant_message` must be read from the conversation's `currentNode` plus `mapping` state under the canonical conversation key. The patch discovers both state atoms and the upstream local→canonical conversation resolver structurally, resolves the renderer's local conversation ID first, then reads the current assistant message. Older injected notifiers using `projectId`/`title` state or direct unaliased `currentNode`/`mapping` reads are migrated to the V2 resolver-aware form.
