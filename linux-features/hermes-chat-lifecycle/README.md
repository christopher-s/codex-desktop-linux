...
Successful completion ordering is deliberate: the renderer sends Hermes `complete_turn` only after the entire native ChatGPT success comma-chain has finished, including stream-controller cleanup, `onStreamComplete`, telemetry finalization, conversation cleanup, request-state cleanup, and queued-draft/composer finalization. The patch locates that native `completed && (...)` chain structurally and appends a versioned terminal hook at its tail. Already-patched candidates with notify-before-cleanup or mid-chain terminal hooks are migrated structurally and idempotently, while malformed or ambiguous V2 placement fails closed. Live E1 also proved that ordering alone does not populate assistant history. `assistant_message` must be read from the conversation's `currentNode` plus `mapping` state under the canonical conversation key. The patch discovers both state atoms and the upstream local→canonical conversation resolver structurally, resolves the renderer's local conversation ID first, then reads the current assistant message. Older injected notifiers using `projectId`/`title` state or direct unaliased `currentNode`/`mapping` reads are migrated to the V2 resolver-aware form.
