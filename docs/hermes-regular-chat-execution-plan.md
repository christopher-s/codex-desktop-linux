...
- The QA LCM reader now forces `messages NOT INDEXED` for logical message counts/rows so secondary-index corruption cannot masquerade as FTS divergence; the harness is **7/7 pass** after this correction. Full E6 acceptance remains blocked only on obtaining a clean live LCM database run. No live DB repair has been performed.
