...
- The same E6 run remains globally **FAIL** because `LCMDatabase.integrity()` detected a pre-existing SQLite tree double-reference and an 8-row FTS/message mismatch before the restart (`307873` messages vs `307865` FTS rows). The identity assertions completed successfully before the integrity gate. Phase B source identity behavior is live-proven; full E6 acceptance stays blocked until LCM integrity is understood and a clean database run is available.
