...
Status: **IN PROGRESS — E1/E2 CLOSED/PASS; E4/E9 pending**
...
- [x] Unit/structural tests. Current lifecycle suite: **26/26 pass**; local-function structural suite: **12/12 pass**; QA harness: **11/11 pass**.
- [x] E1 live run. **PASS:** `20260911T013528Z-e1-no-tool-lifecycle-c8bb1287`; two no-tool turns, one lifecycle session/task, exact server-ID remount recovery after a pristine-upstream-reproduced composer detach, four canonical LCM rows, zero tools/server-key rows, clean integrity, restart/reopen, and read-only postconditions. **Current-product regression PASS:** `20260911T153737Z-e1-no-tool-lifecycle-aa5fb7ac` on ASAR `6d3dde162a52628a4cad482513e7e785db49de0076232bbf63a0024e206c7d06`; the known turn-1 composer detach recovered by exact server ID, both turns completed full lifecycle, restart/exact-ID reopen succeeded, and read-only postconditions remained mutation-free.
- [x] E2 live run. **PASS:** `20260911T071625Z-e2-direct-local-tool-71501845`; exactly one direct `hermes_read_file`, exact client/lifecycle/LCM call-ID correlation, one canonical tool call/result pair, final accepted `completed:true` viewer state, one turn-scoped native `Continued in Work` disclosure, four canonical LCM rows, zero server-key rows, and clean integrity.
- [ ] E4, E9 live runs.
